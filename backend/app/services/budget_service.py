...
class BudgetService:
    ...
    @staticmethod
    def get_budget_alerts(db: Session, user_id: int, threshold: float = 80.0) -> List[Budget]:
        """
        获取预算预警列表（使用率超过阈值的预算）
        ...
        alert_budgets = []
        for budget in budgets:
            spent = BudgetService._calculate_spent(db, budget)
